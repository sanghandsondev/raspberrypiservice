...
DBUS_CONF_FILE="com.example.coremanager.conf"
...
# 1. Install required runtime packages on Pi (idempotent — skips if already installed)
...
ssh "${PI_USER}@${PI_HOST}" "
    ...
    # Use -dev meta-packages for Boost so apt resolves the correct version automatically
    PACKAGES='dbus libdbus-1-3 libsystemd0 libboost-system-dev libboost-filesystem-dev libsqlite3-0'
    MISSING=''
