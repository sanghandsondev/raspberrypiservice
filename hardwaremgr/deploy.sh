...
PROJECT_NAME="hardwaremanager"
SERVICE_FILE="hardwaremanager.service"
DBUS_CONF_FILE="com.example.hardwaremanager.conf"
...
ssh "${PI_USER}@${PI_HOST}" "
    ...
    # Core runtime packages + Bluetooth + oFono + OBEX PBAP
    PACKAGES='dbus libdbus-1-3 libsystemd0 bluez bluez-obex ofono dbus-user-session'
    MISSING=''
    ...
    # Enable user lingering for user '${PI_USER}' so that systemd --user services
    # (including dbus-user-session) are available even without a login session.
    # This is required for ObexPbapClient which uses DBUS_BUS_SESSION.
